...
class Proxy
  ...
  def method_missing( method_name, *args_array, &a_proc)
    @messages[method_name.to_sym] += 1
    if args_array.empty?
      @object.__send__( method_name.to_sym)
    else
      @object.__send__( method_name.to_sym, *args_array )
    end
  end
